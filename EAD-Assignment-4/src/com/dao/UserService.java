...
public class UserService {
	...
	public static List<UserDetails> getFriends(String email) {
		...
		try (Connection conn = ConnectionFactory.getconnection();
			...
			try {
				statement.setString(1, email);
				ResultSet resultSet = statement.executeQuery();
				List<UserDetails> friends = new ArrayList<UserDetails>();
				while (resultSet.next()) {
					UserDetails friend = new UserDetails();
					friend.setFirstName(resultSet.getString("first_name"));
					friend.setLastname(resultSet.getString("last_name"));
					friend.setEmailId(resultSet.getString("email_id"));
					friend.setConNumber(resultSet.getString("contact_number"));
					friend.setOrgName(resultSet.getString("org_name"));
					friends.add(friend);
				}
				return friends;
			} catch (SQLException ex) {
	...
	public static UserDetails getFriendProfile(String email){
		String queryString = QueryFactory.getFriendProfile;
		try (Connection conn = ConnectionFactory.getconnection();
				PreparedStatement statement = conn
						.prepareStatement(queryString);){
			try{
				...
				friend.setConNumber(rset.getString("contact_number"));
				friend.setEmailId(rset.getString("email_id"));
				return friend;
			}catch(SQLException ex){
				System.out.println(ex);
			}
